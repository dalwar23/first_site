<?php
	...
	if($icode1 && $icode2 && $icode3 && $icode4)
	{
		...
		{
			{ header("Location:mverify.php?flg=brown"); exit; }
		}
		else
		{
			//check for the data base entry
				 $query = "SELECT password FROM users WHERE username = '$musr'";
				...
				if ($row = mysql_fetch_assoc($result)) 
				{
					...
					{
						{ header("Location: mverify.php?flg=gray"); exit; }
					}
					else
					{	
				...
				{
					{ header("Location: mverify.php?flg=darkgray"); exit ;}
				}
		}
	}
	else
	{
		{ header("Location:mverify.php?flg=pink"); exit; }
	}
